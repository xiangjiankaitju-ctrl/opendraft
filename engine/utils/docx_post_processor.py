...
def _insert_cover_page(doc: Document, options: dict[str, Any], language: str) -> None:
    ...
    if language == "zh":
        ...
        lines.append(("使用声明：本内容仅供资料梳理和写作参考，不构成正式学术成果。", 10, False))
    else:
        lines = [(title, 18, True)]
        if project_type:
            lines.append((str(project_type), 12, False))
        ...
        lines.extend([
            ("Disclaimer", 10, True),
            ("This content is for research organization and writing reference only.", 10, False),
        ])
...
def _validate_phase_one_docx(doc: Document, language: str, markdown_tables_detected: int, stats: dict[str, Any]) -> None:
    ...
    residue_patterns = [
        ...
        r"\bcite_\d{3,}\b",
        r"\{\s*\([^{}\n]+?\)\s*\}",
        r"(?m)^\s*(?:Title|题目\s*[:：]?|Generated by\s*[:：]?.*|生成工具\s*[:：]?.*)\s*$",
        r"\bTable\s+\d+\s*\|",
        r"\b表\s*\d+\s*\|",
        r"\bMain Body\b",
    ]
...
def _is_cover_or_structural_line(text: str) -> bool:
    return bool(
        re.match(r"^(?:关键词|使用声明|Disclaimer|This content is for research organization)", text)
